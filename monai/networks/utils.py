...
def normalize_transform(shape, device, dtype, align_corners=False):
    ...
    shape_ = list(shape)[::-1]
    if align_corners:
        norm = torch.as_tensor(shape_ + [1.], dtype=dtype, device=device)
        norm[:-1] = 2. / (norm[:-1] - 1.)
        ...
        norm[:-1] = 2. / norm[:-1]
        norm = torch.diag(norm)
        norm[:-1, -1] = 1. / torch.as_tensor(shape_, dtype=dtype, device=device) - 1.
...
def to_norm_affine(affine, src_size, dst_size, align_corners=False):
    ...
    if sr != len(src_size) or sr != len(dst_size):
        raise ValueError('affine suggests a {}D transform, but sizes are src_size={}, dst_size={}'.format(
            sr, src_size, dst_size))
